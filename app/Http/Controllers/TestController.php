...
class TestController extends LfmController
{
    ...
    public function upload()
    {
        ...
        // get the current path to save in db correctlly
        $full_path = parent::getCurrentPath($new_filename);
        $public_path = strpos($full_path, '/public/photos/');
        $real_path = substr($full_path, $public_path);
        if (count($error_bag) == 0) {
            foreach ($files as $name) {
                $post = new Post();
                $post->image = $real_path . '/' . $this->hashFileName($name);
                $post->save();
            }
        }
    ...
    */
    ...
    private function proceedSingleUpload($file)
    {
        ...
        // Our Hash name
        $new_filename = $this->hashFileName($file);
